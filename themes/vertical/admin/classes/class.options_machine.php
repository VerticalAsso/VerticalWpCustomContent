...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = stripslashes((string) $smof_data[$value['id']]);
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes((string) $smof_data[$value['id']]);			
				...
				case 'heading':
					...
					$header_class = str_replace(' ','',strtolower((string) $value['name']));
					$jquery_click_hook = str_replace(' ', '', strtolower((string) $value['name']) );
					$jquery_click_hook = "of-option-" . trim((string) preg_replace('/ +/', '', (string) preg_replace('/[^A-Za-z0-9 ]/', '', urldecode(html_entity_decode(strip_tags($jquery_click_hook))))));
				...
				case 'sliderui':
					...
					$s_val  = stripslashes((string) $smof_data[$value['id']]);
				...
				case "media":
					...
					if($smof_data[$value['id']]){
						$u_val = stripslashes((string) $smof_data[$value['id']]);
					}
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		if (!empty($val['title'])) {
			$slider .= '<li><div class="slide_header"><strong>'.stripslashes((string) $val['title']).'</strong>';
		} else {
		...
		$slider .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][title]" id="'. $id .'_'.$order .'_slide_title" value="'. stripslashes((string) $val['title']) .'" />';
		...
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes((string) $val['description']).'</textarea>';
